...
function [frame_array, segSNR_array] = processing_frames(frame_array, noise_abs2, sub_noise_abs2, Fs)
    ...
    for i = 1:N
        [frame_array(:, i), new_noise_abs2, new_sub_noise_abs2, segSNR_array(i)] = ...
            processing_frame(frame_array(:, i), noise_abs2, sub_noise_abs2, Fs);
        if ~isempty(new_noise_abs2)
            % новая оценка мощности шума
            noise_abs2 = new_noise_abs2;
            sub_noise_abs2 = new_sub_noise_abs2;
        end
    end
end